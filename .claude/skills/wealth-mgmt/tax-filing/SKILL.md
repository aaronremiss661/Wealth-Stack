...
version: 1.0.0
description: |
  Income tax filing assistant for Indian taxpayers. Old vs new regime comparison,
  Section 80C/80D/80G/80E optimization, HRA exemption calculator, capital gains
  summary (LTCG/STCG from equity, debt, property), advance tax deadlines, and
  ITR form selection. Use when asked to "file taxes", "old vs new regime",
  "80C planning", "tax saving", "ITR help", or "advance tax".
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
bash ${CLAUDE_SKILL_DIR}/../bin/tax-calculator.sh --income <amount> --regime both
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/../bin/tax-calculator.sh --capital-gains --portfolio ~/.wealth-mgmt/portfolios/latest.json
```
